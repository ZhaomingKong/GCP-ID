function [denoised_video] = CMStSVD_raw_video_with_global_noise_est_mex(noisy_video, ps, N_step_spatial, N_step_time, SR, maxK, sigma, tau, global_learning)
...
sigma_input_as_mean = mean(noise_est_matrix(:));
...
info2 = int32(info2);
modified = 1;
info3 = [tau,modified,sigma_input_as_mean/255*1.58];


tic;denoised_MSTSVD = color_video_denoising_global_Eigen_global_est_noise_RGGB(single(noisy_video_enlarge),single(U_4D),single(V_4D),info1,info2,single(info3));toc;
