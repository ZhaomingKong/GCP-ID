function [im_denoised] = GCP_CID_sRGB_color_video_with_mex_enlarge(im1, noise_lvl_mtx, global_est, ps, maxK, SR, N_step_spatial, N_step_time, tau, divide_factor)
%% The following code implements GCP_CID with mex functions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
noisy_video = im1;
[H_origin, W_origin, C, D] =  size(noisy_video);
...
for k = 1:N_step_time:D
    for i=1:N_step_spatial:H-ps+1
        for j=1:N_step_spatial:W-ps+1
            ...
            patch_ij = squeeze(noisy_video_enlarge(i:i+ps-1,j:j+ps-1,:,k));
...
frame_chosen = 3;
im_frame = noisy_video_enlarge(:,:,:,frame_chosen);
...
if(global_est == 1)   
    im2 = GCP_CID_video_denoising(single(noisy_video_enlarge), int32(green_mode_all), single(U_3D_fft),single(V_3D_fft), single(U_4D_fft),single(V_4D_fft), int32(info1), int32(info2), single(info3));
    im_denoised = mat_ten(im2,1,size(noisy_video_enlarge));
    im_denoised = im_denoised(1:H_origin, 1:W_origin, :, :);
elseif(global_est == 0)
    im2 = GCP_CID_video_denoising_local_neighbour_noise_est(single(noisy_video_enlarge), int32(green_mode_all), single(U_3D_fft), single(V_3D_fft), single(U_4D_fft), single(V_4D_fft), int32(info1), int32(info2), single(info3), single(noise_lvl_vectors));
    im_denoised = mat_ten(im2,1,size(noisy_video_enlarge));
    im_denoised = im_denoised(1:H_origin, 1:W_origin, :, :);
end
