...
N_step = 8; % Nstep --> could be 4/6/8. Choose a smaller size for slightly better results. Choose a larger one for faster speed. 
maxK = 120; % number of patches. Choose a larger one for slightly better results. Choose a smaller one for faster speed. 
divide_factor = 1.2; % controls the seach scheme
modified = 1; % won't do anything for mex function
tau = 1.1; % threshold factor
global_learning = 0; % set 0/1 for local/global estimate
...
% % load est matrix
file_name = 'noise_lvl_est_SIDDvalidation.mat'; %% noise_lvl_est_mtx_rawDND_modified
load(file_name);
disp('------------------------------------------------------')
disp(file_name)
disp('------------------------------------------------------')
...
for i = 1:N
    ...
    for b = 1:B
        ...
        [im_denoised_raw_origin] = GCP_ID_raw_est_CNN(noisy_ib, clean_ib, noise_lvl_mtx_ib, ps, SR, maxK, N_step, modified, tau, global_learning);
        Time_ib = toc;
...
% load noise_lvl_est_SIDDbenchmark_modified.mat %% noise_lvl_est_SIDDbenchmark or noise_lvl_est_SIDDbenchmark_modified
...
%% Test DND dataset
% clear all;
% noise_est_name = 'noise_lvl_est_mtx_rawDND_modified.mat'; %% noise_lvl_est_mtx_rawDND.mat
% load(noise_est_name);
% disp('###########################################')
% disp(noise_est_name)
% disp('###########################################')
...
% % Parameter setting
% disp('--------------------------------------------------------------')
% disp(['denoising_method: ',num2str('GCP-ID')])
...
%         
%         tic
%         [im_denoised_raw_origin] = GCP_ID_raw_est_CNN(noisy_test, origin_test, noise_lvl_mtx_ib, ps, SR, maxK, N_step, modified, tau, global_learning);
%         time = toc;
% 
